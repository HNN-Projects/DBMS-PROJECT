...
if(isset($_POST['app-submit']))
{
  ...
  $pid = $_SESSION['pid'];
  $ID = isset($_SESSION['ID']) ? $_SESSION['ID'] : ''; // Check if 'ID' is set
  $username = $_SESSION['username'];
  $emailSession = $_SESSION['email']; // Renamed to avoid conflict with $email below
  $fname = $_SESSION['fname'];
  $lname = $_SESSION['lname'];
  $gender = $_SESSION['gender'];
  $contact = $_SESSION['contact'];
  $doctor = $_POST['doctor'];
  $email = $emailSession; // Assign the value from session to $email
  // $fees = $_POST['fees']; // Uncomment if needed
    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"SELECT apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          $query = mysqli_query($con, "INSERT INTO appointmenttb (pid, ID, fname, lname, gender, email, contact, doctor, docFees, appdate, apptime, userStatus, doctorStatus) VALUES ('$pid', '$ID', '$fname', '$lname', '$gender', '$email', '$contact', '$doctor', '$docFees', '$appdate', '$apptime', '1', '1')");
